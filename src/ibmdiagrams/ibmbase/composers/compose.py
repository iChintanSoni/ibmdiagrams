...
class Compose:
    ...
    def __init__(self, common, data):
        ...
        self.diagramname = path.splitext(
            path.basename(self.common.getOutputFile()))[0]
    ...
    def composeDiagrams(self):
        ...
        if self.common.isDrawioCode():
            result = subprocess.run(
                [sys.executable, filelocation], capture_output=True, text=True)
            remove(filelocation)
    ...
    def composeTree(self):
        ...
        diagram = TreeNode("Diagram Group", randomid(),
                           outputfolder, 'LR', fontname, frame.iloc[0])
    ...
    def composeIconTree(self, top):
        ...
        for childName, childIcon in iconDictionary.items():
            ...
            for childKey, childRow in childData.iterrows():
                childID = childRow['id']
                childNode = TreeNode(childName, childID,
                                     None, childDirection, None, childRow)
            ...
            if deployedOnID == deployedOnRow['id']:
                if deployedToGroup != 'none':
                    childNode = self.insertDeployedTo(
                        childNode, deployedToData, deployedToGroup, deployedToID, deployedToDirection)
                ...
                if deployedOnID in self.savegroups:
                    ...
                    deployedOnNode = TreeNode(
                        deployedOnGroup, deployedOnID, None, deployedOnDirection, None, deployedOnRow)
                    self.savegroups[deployedOnID] = deployedOnNode
                    deployedOnNode.addNode(childNode)
                    if nextDeployedOnGroup != "none":
                        deployedOnNode = self.composeGroupTree(
                            deployedOnNode, deployedOnIcon)
                break
            ...
            if deployedToID == deployedToRow['id']:
                if deployedToID in self.savegroups:
                    ...
                    deployedToNode = TreeNode(
                        deployedToGroup, deployedToID, None, deployedToDirection, None, deployedToRow)
        ...
        if index != -1:
            ...
            for child in parent.children:
                ...
                if name in combinedIcons:
                    ...
                    newdata["id"] = newid
                    combinedIcons[name] = {
                        "name": newname, "id": newid, "direction": newdirection, "fontname": newfontname, "data": newdata}
                else:
                    combinedIcons[name] = {
                        "name": name, "id": id, "direction": direction, "fontname": fontname, "data": data}
            ...
            for name in combinedIcons:
                entry = combinedIcons[name]
                node = TreeNode(
                    name, entry["id"], None, entry["direction"], None, entry["data"])
                parent.addNode(node)
        else:
            for child in parent.children:
    ...
    def composeResources(self, parent, useCustomLabel, pythonfile):
        ...
        if index != -1:
            ...
            print(" " * self.indent + "with " + name + "(" + repr(str(label)) +
                  sublabel + output + direction + fontname + "):", file=pythonfile)
        ...
        if index != -1:
            ...
            print(" " * self.indent + name + "(" + repr(str(label)) +
                  sublabel + fontname + ")", file=pythonfile)
