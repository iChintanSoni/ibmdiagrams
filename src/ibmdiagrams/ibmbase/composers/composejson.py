...
from ..utils.constants import ComponentFill, FillPalette, ShapeKind, ShapeName, ShapePos, ZoneCIDR
...
class ComposeJSON:
    ...
    def __init__(self, common, data):
        ...
        self.cloudname = ShapeName.CLOUD.value if self.common.isProviderAny(
        ) else ShapeName.IBM_CLOUD.value
    ...
    def composeDiagrams(self):
        ...
        if self.common.isInputTerraform():
            ...
            if self.data.hasServices():
                ...
                self.properties.updateSequence(
                    self.common.compress(resourceid))
                groups, items, connectors = self.composeResourceGroup(
                    regionid, groups, items, connectors, resourceid, resourcename)

                servicesid = randomid()
                self.properties.updateSequence(
                    self.common.compress(servicesid))
                groups, items, connectors = self.composeServices(
                    groups, items, connectors, servicesid, resourceid)
                ...
                iconname, linecolor, fillcolor = self.icons.getIcon(
                    'Region Group')
                properties = self.properties.getGroupProperties(
                    label=regionname, linecolor=linecolor, fillcolor=fillcolor, icon='Region Group', shape='ploc', direction='TB', parentid=self.common.compress(cloudid))
        ...
        for vpcindex, vpcframe in vpcs.iterrows():
            ...
            if not self.common.isInputTerraform():
                ...
                internetid = randomid()
                self.properties.updateSequence(self.common.compress(publicid))
                groups, items, connectors = self.composePublic(
                    groups, items, connectors, publicid, internetid)
            ...
            groups, items, connectors = self.composeLoadBalancers(
                vpcname, vpcid, groups, items, connectors, internetid)
            ...
            self.properties.updateSequence(
                self.common.compress(publicconnectorid))

            for regionzonename in vpcTable[vpcid]:
                zoneid = self.common.compress(regionzonename)
                self.properties.updateSequence(zoneid)

                groups, items, connectors = self.composeSubnets(
                    regionzonename, vpcname, groups, items, connectors, internetid)
                ...
                properties = self.properties.getGroupProperties(label=zonename, sublabel=zonecidr, linecolor=linecolor, fillcolor=fillcolor,
                                                                icon='Availability Zone Group', shape='zone', direction='TB', parentid=self.common.compress(vpcid))
            ...
            if self.common.isInputTerraform():
                self.properties.updateSequence(
                    self.common.compress(vpcresourceid))
                groups, items, connectors = self.composeResourceGroup(
                    regionid, groups, items, connectors, vpcresourceid, vpcresourcename)
                iconname, linecolor, fillcolor = self.icons.getIcon(
                    'VPC Group')
                properties = self.properties.getGroupProperties(
                    label=vpcname, linecolor=linecolor, fillcolor=fillcolor, icon='VPC Group', shape='ploc', direction='LR', parentid=self.common.compress(vpcresourceid))
                groups[self.common.compress(vpcid)] = properties

                if self.tgwid != 0:
                    properties = self.properties.getSingleArrowProperties(
                        sourceid=self.common.compress(self.tgwid), targetid=self.common.compress(vpcid))
                    connectorid = randomid()
                    connectors[self.common.compress(connectorid)] = properties
                    self.properties.updateSequence(
                        self.common.compress(connectorid))
            else:
                ...
                iconname, linecolor, fillcolor = self.icons.getIcon(
                    'VPC Group')
                properties = self.properties.getGroupProperties(
                    label=vpcname, linecolor=linecolor, fillcolor=fillcolor, icon='VPC Group', shape='ploc', direction='LR', parentid=self.common.compress(regionid))
                groups[self.common.compress(vpcid)] = properties
            ...
            iconname, linecolor, fillcolor = self.icons.getIcon(
                'IBM Cloud Group')
            properties = self.properties.getGroupProperties(
                label='IBM Cloud', linecolor=linecolor, fillcolor=fillcolor, icon='IBM Cloud Group', shape='ploc', parentid=None)
            ...
            if not self.common.isInputTerraform():
                properties = self.properties.getDiagramProperties(
                    name=vpcname, filename='*')
                # diagramid = randomid()
        ...
        if validdata:
            self.shapes.dumpXML(self.common.getOutputFile(),
                                self.common.getOutputFolder())
    ...
    def composeServices(self, groups, items, connectors, servicesid, parentid):
        ...
        properties = self.properties.getGroupProperties(label='Cloud Services', linecolor=linecolor, fillcolor=fillcolor,
                                                        icon='Cloud Services Group', shape='ploc', direction='TB', parentid=self.common.compress(parentid))
        ...
        if self.data.hasActivityTrackers():
            ...
            properties = self.properties.getItemProperties(
                label=activityname, icon='Cloud Logs Icon', parentid=self.common.compress(servicesid))
            trackerid = randomid()
        ...
        if self.data.hasKeyManagement():
            ...
            properties = self.properties.getItemProperties(
                label=keyname, icon='Key Protect Icon', parentid=self.common.compress(servicesid))
            keymgmtid = randomid()
        ...
        if self.data.hasObjectBuckets():
            ...
            properties = self.properties.getItemProperties(
                label=objectname, icon='Object Storage Icon', parentid=self.common.compress(servicesid))
            objectid = randomid()
        ...
        if self.data.hasFlowLogs():
            ...
            properties = self.properties.getItemProperties(
                label=logname, icon='Flow Logs Icon', parentid=self.common.compress(servicesid))
            flowlogsid = randomid()
        ...
        if self.data.hasTransitGateways():
            ...
            properties = self.properties.getItemProperties(
                label=transitname, icon='Transit Gateway Icon', parentid=self.common.compress(servicesid))
            self.tgwid = randomid()
    ...
    def composeResourceGroup(self, parentid, groups, items, connectors, resourceid, resourcename):
        ...
        properties = self.properties.getGroupProperties(
            label=resourcename, icon='Resource Group', shape='zone', direction='TB', parentid=self.common.compress(parentid))
    ...
    def composePublic(self, groups, items, connectors, publicid, internetid):
        ...
        properties = self.properties.getGroupProperties(
            label='Public Network', linecolor=linecolor, fillcolor=fillcolor, icon='Public Network Group', shape='ploc', direction='TB', parentid=None)
        ...
        properties = self.properties.getItemProperties(
            label='User', icon='User Icon', shape='actor', parentid=self.common.compress(publicid))
        ...
        properties = self.properties.getItemProperties(
            label='Internet', icon='Internet Icon', parentid=self.common.compress(publicid))

        items[self.common.compress(internetid)] = properties
        self.properties.updateSequence(self.common.compress(internetid))

        properties = self.properties.getDoubleArrowProperties(
            sourceid=self.common.compress(userid), targetid=self.common.compress(internetid))

        internetconnectorid = randomid()
        connectors[self.common.compress(internetconnectorid)] = properties
        self.properties.updateSequence(
            self.common.compress(internetconnectorid))
    ...
    def composeEnterprise(self, groups, items, connectors, enterpriseid):
        ...
        properties = self.properties.getGroupProperties(label='Enterprise Network', linecolor=linecolor,
                                                        fillcolor=fillcolor, icon='Enterprise Network Group', shape='ploc', direction='TB', parentid=None)
        ...
        properties = self.properties.getItemProperties(
            label='User', icon='User Icon', parentid=self.common.compress(enterpriseid))
        userid = randomid()
    ...
    def composeSubnets(self, regionzonename, vpcname, groups, items, connectors, internetid):
        ...
        for insubnetid in zoneTable[regionzonename]:
         ...
         '''
            ...
            groups, items, connectors = self.composeSubnetIcons(
                subnetid, subnetname, subnetvpcname, vpcname, groups, items, connectors, internetid)
            ...
            properties = self.properties.getGroupProperties(label=subnetname, sublabel=subnetcidr, linecolor=linecolor, fillcolor=fillcolor,
                                                            icon='Subnet Group', shape='ploc', direction='TB', parentid=self.common.compress(regionzonename))
            ...
            if pubgatefipip != None:
                ...
                if save_subnetpubgateid == None:
                    ...
                    properties = self.properties.getItemProperties(
                        label=pubgatename, sublabel=pubgatefipip, icon='Public Gateway Icon', parentid=self.common.compress(regionzonename))
                    # pubgateid = randomid()
                    items[self.common.compress(subnetpubgateid)] = properties
                    self.properties.updateSequence(
                        self.common.compress(subnetpubgateid))

                    properties = self.properties.getSingleArrowProperties(sourceid=self.common.compress(
                        subnetid), targetid=self.common.compress(subnetpubgateid))
                    ...
                    # routername = vpcname + '-router'
                    properties = self.properties.getSingleArrowProperties(sourceid=self.common.compress(
                        subnetpubgateid), targetid=self.common.compress(internetid))
    ...
    def composeSubnetIcons(self, subnetid, subnetname, subnetvpcname, vpcname, groups, items, connectors, internetid):
        ...
        for iconframe in icons:
            ...
            if icontype.lower() == 'instance':
                ...
                for nicframe in nics:
                    ...
                    if self.common.isInputTerraform():
                        ...
                        nicinstanceid = instanceframe['id'] if self.common.isInputRIAS(
                        ) else nicframe['instanceId']
                        nicip = nicframe['primary_ip']['address'] if self.common.isInputRIAS(
                        ) else nicframe['ip']
                    if nicips == '':
                        nicips = nicip
                    else:
                ...
                if 'bandwidth' in instanceframe:
                    ...
                    if bandwidth == '' or (isinstance(bandwidth, float) and isnan(bandwidth)):
                        ...
                        instancecpuspeed = int(
                            instanceframe['bandwidth'] / 1000)
                ...
                if nicfipip != None:
                    ...
                    properties = self.properties.getDoubleArrowProperties(
                        label=iplabel, sourceid=self.common.compress(instanceid), targetid=self.common.compress(internetid))

                    fipconnectorid = randomid()
                    connectors[self.common.compress(
                        fipconnectorid)] = properties
                    self.properties.updateSequence(
                        self.common.compress(fipconnectorid))
                ...
                if self.common.isInputTerraform():
                    ...
                    secondarytext = iconframe['private_ip_address'] + \
                        '<br>' + iconframe['public_ip_address']
            ...
            properties = self.properties.getItemProperties(
                label=iconname, sublabel=secondarytext, icon=icontype, data=meta, parentid=self.common.compress(subnetid))
                    ...
                    for lbid, members in lbpool[lbkey].items():
                        ...
                        for member in members:
                            if self.common.isInputRIAS():
                                ...
                                nicdata = self.data.getNetworkInterface3(
                                    memberaddress)
                                ...
                                if instancevpcid == vpcid:
                                    if not lbgenerated:
                                        ...
                                        properties = self.properties.getItemProperties(
                                            label=lbname, sublabel=lbiplist, icon='Load Balancer Icon', parentid=self.common.compress(vpcid))
                                        lbid = randomid()
                                        # lbid = self.common.compress(lbid)
                                        items[self.common.compress(
                                            lbid)] = properties
                                        self.properties.updateSequence(
                                            self.common.compress(lbid))

                                        # routername = vpcname + '-router'
                                        properties = self.properties.getDoubleArrowProperties(
                                            sourceid=self.common.compress(lbid), targetid=self.common.compress(internetid))
                                    ...
                                    properties = self.properties.getDoubleArrowProperties(
                                        sourceid=self.common.compress(instanceid), targetid=self.common.compress(lbid))
                            ...
                            if not self.common.isInputTerraform():
                                nicdata = self.data.getNetworkInterface(
                                    address, instanceid)
                                if len(nicdata) != 0:
                                    nicid = nicdata['id']
                                    nicinstanceid = nicdata['instance.id']
                                    instanceframe = self.data.getInstance(
                                        nicinstanceid)
                                    ...
                                    if instancevpcid == vpcid:
                                        if not lbgenerated:
                                            ...
                                            meta = {}
                                            meta = meta | {
                                                'Type': "public" if lbispublic else "private"}
                                            meta = meta | {'IPs': lbiplist}
                                            properties = self.properties.getItemProperties(
                                                label=lbname, sublabel=lbiplist, icon='Load Balancer', data=meta, parentid=self.common.compress(vpcid))
                                            lbid = randomid()
                                            # lbid = self.common.compress(lbid)
                                            items[self.common.compress(
                                                lbid)] = properties
    ...
    def composeNetworkACLs(self, vpcname, vpcid, groups, items, connectors, internetid):
        ...
        if acls != None:
            for acl in acls:
                ...
                properties = self.properties.getItemProperties(
                    label=aclname, icon='ACL Rules', data=meta, parentid=self.common.compress(vpcid))
