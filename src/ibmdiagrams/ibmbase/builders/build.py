...
from ..utils.enums import Alternates, GroupTypes, Directions, ConnectorArrows, ConnectorStyles, ItemTypes, OutputFormats, FontNames, Providers
...
class Build:
    ...
    def buildSheets(self, properties, diagrams):
        ...
        if self.common.isInputPython():
            self.common.printDone(
                path.join(outputfolder, outputfile), provider)
    ...
    def buildAll(self):
        ...
        for connectorid, properties in self.connectors.items():
            connectoritems, connectorlinks, connectorvalues = self.buildConnectorShape(
    ...
    def buildGroups(self, groupids):
        ...
        for groupid in groupids:
            ...
            groupitems, grouplinks, groupvalues = self.buildGroupShape(
                groupid, properties)
            items += groupitems
            links += grouplinks
            values += groupvalues
            ...
            if len(childids) > 0:
                groupitems, grouplinks, groupvalues = self.buildGroups(
                    childids)
                items += groupitems
                links += grouplinks
                values += groupvalues
    ...
    def buildGroupShape(self, groupid, properties):
        ...
        shapenode = self.shapes.buildDrawioShape(
            groupid, properties, x, y, width, height, meta, genflag)
    ...
    def buildConnectorShape(self, connectorid, properties):
      ...
      '''
        ...
        if startarrow == "" and endarrow == "":
            connectornode = self.shapes.buildSolidLink(connectorid, label, sourceid, targetid, startarrow,
                                                       endarrow, startfill, endfill, linetype, linewidth, linecolor, fontname, fontsize, None)
        elif startarrow != "" and endarrow != "":
            connectornode = self.shapes.buildDoubleArrow(
                connectorid, label, sourceid, targetid, startarrow, endarrow, startfill, endfill, linetype, linewidth, linecolor, fontname, fontsize, None)
        # elif startarrow != "" or endarrow != "":
        else:
            connectornode = self.shapes.buildSingleArrow(
                connectorid, label, sourceid, targetid, startarrow, endarrow, startfill, endfill, linetype, linewidth, linecolor, fontname, fontsize, None)
    ...
    def sequenceChildren(self):
        ...
        for groupid, properties in self.groups.items():
            ...
            for childid in childids:
                templist.append(
                    {"id": childid, "index": self.sequence.index(childid)})
    ...
    def calculateIsolatedItems(self):
        ...
        for groupid, properties in self.groups.items():
            ...
            groupwidth = (minnodewidth + (2 * minitemspace)
                          ) if direction == "TB" else minitemspace
    ...
    def calculateLeftRightGroups(self, parentid):
        ...
        for childids in parentchildren:
            ...
            if position == 1:
                ...
                saveheight = max(
                    saveheight, childgeometry[3] + (2 * minshapespace))
                if childfinal:
                    self.groups[childid]["geometry"] = [
                        savex, savey, childwidth, childheight]
                    newparentwidth += childwidth + savex
                    newparentheight += childheight + savey
                else:
                    self.groups[childid]["final"] = True
                    self.groups[childid]["geometry"] = [
                        savex, savey, savewidth, saveheight]
    ...
    def calculateTopBottomGroups(self, parentid):
        ...
        for childids in parentchildren:
            ...
            if position == 1:
                ...
                savey = mintopspace
                savewidth = max(
                    savewidth, childgeometry[2] + (2 * minshapespace))
                saveheight += childheight
                if childfinal:
                    self.groups[childid]["geometry"] = [
                        savex, savey, childwidth, childheight]
                ...
                savex = minshapespace
                savey = saveheight + mintopspace + \
                    ((position - 1) * minshapespace)
                savewidth += childwidth
                saveheight += childheight
                if childfinal:
                    self.groups[childid]["geometry"] = [
    ...
    def calculateLeftRightItems(self, parentid, parentgeometry):
        ...
        for childids in parentchildren:
            ...
            if isinstance(childids, list):
                ...
                minimalwidth = (2 * minshapespace) + (nodecount *
                                                      minnodeheight) + (nodecount * minitemspace)
                ...
                for childid in childids:
                    ...
                    self.groups[childid]["geometry"] = [
                        childx, childy, minnodewidth, minnodeheight]
                ...
                adjustwidth = max(
                    adjustwidth, minitemspace + (nodeindex * minnodewidth) + (nodeindex * minitemspace))
        ...
        if adjustheight > 0:
            newparentheight += adjustheight + \
                (itemsets * minshapespace) + (2 * minshapespace)
    ...
    def calculateTopBottomItems(self, parentid, parentgeometry):
        ...
        for childids in parentchildren:
            ...
            if isinstance(childids, list):
                ...
                nodeindex = 0
                nodecount = len(childids)
                minimalheight = mintopspace + minshapespace + \
                    (nodecount * minnodeheight) + (nodecount * minitemspace)
                ...
                adjustheight = max(
                    adjustheight, mintopspace + (nodeindex * minnodeheight) + (nodeindex * minitemspace))
