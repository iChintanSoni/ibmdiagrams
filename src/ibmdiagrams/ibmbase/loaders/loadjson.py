...
class LoadJSON:
    ...
    # Table of nics ordered by subnet+instance that shows nics for same instance in different subnets.
    nicTable = {}
    # Table of subnet icons ordered by subnet that shows subnet icons within each subnet.
    subnetIconTable = {}
    # Table of service icons ordered by region that shows service icons within each region.
    serviceIconTable = {}
    # Table of vpcs ordered by region that shows vpcs within each region.
    regionTable = {}
    # Table of zones ordered by vpc that shows zones with each vpc.
    vpcTable = {}
    # Table of subnets ordered by vpc+zone that shows subnets within each zone.
    zoneTable = {}
    # Table of load balancerbs ordered by vpc+lb that shows load balancers within each vpc.
    lbTable = {}
    # Table of network ACLs ordered by vpc that shows network ACLs within each vpc.
    aclTable = {}
    ...
    def analyzeInstances(self):
        ...
        if not instances.empty:
            for instanceindex, instanceframe in instances.iterrows():
                ...
                addedInstance = False
                nics = instanceframe['network_interfaces'] if self.common.isInputRIAS(
                ) else instanceframe['networkInterfaces']
                if nics:
                    for nicframe in nics:
                        ...
                        if self.common.isInputTerraform():
                            ...
                            nicsubnetid = nicframe['subnet']['id'] if self.common.isInputRIAS(
                            ) else nicframe['networkId']

                        if nicsubnetid in self.subnetIconTable:
                            if addedInstance == False:
                                self.subnetIconTable[nicsubnetid].append(
                                    instanceframe)
                                addedInstance = True
                        else:
                    ...
                    for vpeipindex, vpeipframe in vpeips.iterrows():
                        ...
                        if vpesubnetid in self.subnetIconTable:
                            if addedVPE == False:
                                ...
                                self.subnetIconTable[vpesubnetid].append(
                                    vpeframecopy)
                                addedVPE = True
                            else:
                            ...
                            for lbmember in lbmembers:
                                if lbmember:
                                    ...
                                    if vpcid == None:
                                        ...
                                        if instanceIP != None:
                                            instance = self.getInstanceByIP(
                                                instanceIP)

                                            if len(instance) == 0:
                                                vpcid = None
                                                self.common.printInvalidInstanceMember(
                                                    lbname, lbpoolname, instanceId)
                                                continue
